...
router.put("/:drinkId", authenticate, async (req, res, next) => {
  ...
  await Wish.updateOne(
    ...
    .then(() => {
      return Wish.findOne({ drinkId: drinkId, userId: userId }).populate(
        "drinkId"
      );
    })
    .then((updatedData) => {
...
/* POST : 해당 술에 대한 나의 리뷰 생성(review===null이면 위시만 있는 상태) */
router.post("/:drinkId", authenticate, async (req, res, next) => {
  ...
  await Wish.findOne({
    ...
    if (data) {
        res.json({ result: false, message: "already exist wish"});
    } 
    else {
        Wish.create({
            drinkId: drinkId,
            ...
            res.json({ result: false, message: "not create wish"});
        });
    }
  }).catch(() => {
    res.json({ result: false, message: "not find drink" });
  });
});
